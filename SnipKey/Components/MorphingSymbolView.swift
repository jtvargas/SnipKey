...
struct MorphingSymbolView: View {
    ...
    struct Config {
        ...
        var symbolAnimation: Animation = .smooth(duration: 0.5, extraBounce: 0.5)
    }
}

#Preview {
    MorphingSymbolView(symbol: "paintbrush.pointed.fill", config: .init(font: .system(size: 100), frame: CGSize(width: 250, height: 200), radius: 15, foregroundColor: .black))
}
